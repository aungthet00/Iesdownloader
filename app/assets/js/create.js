$(document).ready(function () {
	...
	$("#lamp").on("change", function (e) {
		...
		const selectedFixtureOption = $("#fixture").find(":selected");
		const fixtureValue = selectedFixtureOption.val();
		if (fixtureValue !== "") {
			$.ajax({
				url: `${siteurl}dashboard/get_accessories/${valueSelected}/${fixtureValue}`,
				method: "GET",
				success: function (response) {
					response = JSON.parse(response);
					if (response.status === 200) {
	...
	$("#accessories").change(function () {
		...
		if (
			...
			$("#errormsg").text(
				"Select 3 accessories if Comfort Pro is selected otherwise select 2."
			);
